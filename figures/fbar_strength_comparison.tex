...
\addplot[only marks, mark=*, mark size=3pt, red] table[
    ...
    y=rel_err_first,
    meta=nu,
]{data/nu_sweep/strength/fbar_strength_results_old.csv};
...
\addplot[only marks, mark=*, mark size=3pt, blue] table[
    ...
    y=rel_err_first,
    meta=nu,
]{data/nu_sweep/strength/fbar_strength_results_new.csv};

\end{groupplot}
\end{tikzpicture}
\caption{Dynamic simulation - Relative error at target vs.\ maximum F-bar pressure correction for method \#1 (left) and method \#2 (right), with Poisson's ratio $\nu$ labelled at each point.}
