...
\addplot[only marks, mark=*, mark size=3pt, red] table[
    ...
    y=rel_err_target,
    meta=nu,
]{data/nu_sweep/strength/fbar_strength_results_elastic_old.csv};
...
\addplot[only marks, mark=*, mark size=3pt, blue] table[
    ...
    y=rel_err_target,
    meta=nu,
]{data/nu_sweep/strength/fbar_strength_results_elastic_new.csv};

\end{groupplot}
\end{tikzpicture}
\caption{Dynamic-elastic simulation: Relative error at target vs.\ maximum F-bar pressure correction for method \#1 (left) and method \#2 (right), with Poisson's ratio $\nu$ labelled at each point.}
\label{fig:fbar_strength_comparison_elastic}
\end{figure}
