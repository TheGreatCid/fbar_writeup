...
\addplot[only marks, mark=*, mark size=3pt, red] table[
    ...
    y=rel_err_first,
    meta=bc_case,
]{data/bc_sweep/strength/bc_sweep_results_elastic_quasi_old.csv};
...
\addplot[only marks, mark=*, mark size=3pt, blue] table[
    ...
    y=rel_err_first,
    meta=bc_case,
]{data/bc_sweep/strength/bc_sweep_results_elastic_quasi_new.csv};

\end{groupplot}
\end{tikzpicture}
\caption{BC sweep, quasi-static-elastic case: Relative error at target vs.\ maximum F-bar pressure correction for method \#1 (left) and method \#2 (right), with the boundary-condition case labelled at each point.}
